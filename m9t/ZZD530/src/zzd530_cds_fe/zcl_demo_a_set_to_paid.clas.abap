...
CLASS zcl_demo_a_set_to_paid IMPLEMENTATION.
  ...
  METHOD /bobf/if_frw_action~execute.
    ...
    DATA(it_sales_order) = VALUE ztdemo_i_salesorder_tx(  ).
    ...
    io_read->retrieve(
      ...
      IMPORTING
        ...
        et_data                 =  it_sales_order
    ).

    " WRITING BO data ---------------------------------------------
    LOOP AT it_sales_order ASSIGNING FIELD-SYMBOL(<s_sales_order>)   .
      " Set the attribue billing_status to new value
      <s_sales_order>-billingstatus = 'P'.
